...
class Settings(BaseSettings):
    ...
    #: Cuenta de WhatsApp Business a la que pertenece el numero. Solo hace
    #: falta para listar las plantillas aprobadas: enviarlas no la necesita,
    #: porque el envio va contra el numero. Sin esto, la consola no puede
    #: ofrecer plantillas y no deja iniciar conversaciones.
    ...
    # ------------------------------------------ SSO con Google (consola) -----
    #: Alternativa personal a SAML: convive con ella y con el login por
    #: contraseña. Mientras falte cualquiera de los dos valores, el botón
    #: «Iniciar sesión con Google» queda oculto. Google Cloud Console >
    #: APIs y servicios > Credenciales > ID de cliente de OAuth 2.0 (tipo
    #: «Aplicación web»), con `PUBLIC_BASE_URL + /auth/google/callback` como
    ...
    # --------------------------------------------------- correo saliente ----
    #: Notificaciones de cuenta: confirmación de registro del chatbox e
    #: invitación al crear un supervisor o administrador. Mientras falten
    #: usuario o contraseña, `smtp_enabled` es falso: el correo simplemente no
    #: se envía y queda registrado en el log, sin romper el registro ni el
    #: alta de un agente que ya se guardaron en la base.
    smtp_host: str | None = "smtp.gmail.com"
    smtp_port: int = 587
    #: Cuenta de Gmail / Google Workspace remitente. La contraseña debe ser
    #: una "contraseña de aplicación" (myaccount.google.com/apppasswords), no
    #: la contraseña normal de la cuenta: Google la rechaza en SMTP.
    smtp_username: str | None = None
    smtp_password: SecretStr | None = None
    #: Remitente que ve quien recibe el correo. Por omisión, la propia cuenta.
